...
namespace servis_za_ciscenje.Sloj_logike
{
    class Logika : IPoslovnaLogika
    {
                        ...
                        else
                        {
                            ...
                            else
                            {
                                ...
                                else
                                {
                                    ...
                                    posao pos = new posao { adresa_ciscenja = adresa, velicina_prostora = Convert.ToInt32(kvadratura), cijena = cijena, vrijeme = termin, datum = Convert.ToDateTime(datum), tip_posla = tipPosla, klijentid_klijent = idKlijenta };
                                    db.posao.Add(pos);
                                    db.SaveChanges();
        ...
        public void SaljiMail()
        {
            ...
            {
                if (posao.klijent.primanjeObavijesti.Equals("Da"))
                {
                    DateTime datumBaza = posao.datum ?? DateTime.Now;
                    TimeSpan preostaloVrijeme = now.Subtract(datumBaza);
        ...
        public bool checkIfRadniciExist()
        {
            bool exist = false;
            foreach (var item in db.radnici)
            {
